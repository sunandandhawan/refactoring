...
describe("Production Plan", () => {
  describe("Producers", () => {
    ...
    it("should be able to get cost", () => {
      const cost = 9;
      const name = "TestProducerName";
      const producer = new Producer(name);
      producer.cost = cost;
      expect(producer.cost).to.equal(cost);
    });
  ...
  describe("Province", () => {
    ...
    it("should be able to get the demand", () => {
      const demand = 10;
      const name = "TestProvinceName";
      const province = new Province(name);
      province.demand = demand;
      expect(province.demand).to.equal(demand);
    });
    it("should be able to get the price", () => {
      const price = 20;
      const name = "TestProvinceName";
      const province = new Province(name);
      province.price = price;
      expect(province.price).to.equal(price);
    });
    ...
    it("should be able to list all the Producers in the Province", () => {
      const province = new Province("TestProvinceName");
      province.addProducer(new Producer("TestProducerName"));
      province.addProducer(new Producer("TestProducerName"));
      expect(Array.isArray(province.producers)).to.be.true;
    });
